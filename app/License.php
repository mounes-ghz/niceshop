...
use NiceShop\Exceptions\InvalidLicenseException;

class License
{
    private $license;
    private string $licenseFilePath;
    private string $endpoint = 'https://codenevis.net';
    ...
    {
        $this->activate(
            $this->getLicenseFromFile()->username, $this->getLicenseFromFile()->purchase_code
        );
    }
     ...
     * @throws InvalidLicenseException|GuzzleException
     */
    public function activate($username, $purchaseCode): void
    {
        ...
        try {
            $response = $client->post('/api/v1/licenses', [
                'form_params' => [
                    'item_id'       => NiceShop::ITEM_ID,
                    'domain'        => request()->root(),
                    'product_name'  => 'niceshop',
            ...
            if ($response->status === 'success' && !$response->valid) {
                throw new InvalidLicenseException('کد فعال سازی اشتباه است.');
            }
