...
export default class {
        ...
        if (NiceShop.lang == "fa") {
            setTimeout(() => {
                ...
                for (let el of elements) {
                    ...
                    let type = $(el).attr("data-time") ? "datetime" : "date";
                    let value =
                        $(el).val() != ""
                            ? $(el).val()
                            : $(el).attr("data-default-date");
                    let format =
                    ...
                    $(el).replaceWith(
                        `<custom-date-picker name="${name}" type="${type}" value="${value}" />`
                    );
                }

                if ($("custom-date-picker").length) {
                    $("custom-date-picker").each(function (index, el) {
                        new Vue({
                            el: el,
                            components: {
                                CustomDatePicker,
    ...
    shortcuts() {
        Mousetrap.bind("f1", () => {
            window.open("https://docs.niceshop.codenevis.net/", "_blank");
        });
