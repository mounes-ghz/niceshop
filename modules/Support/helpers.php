...
if (!function_exists('is_core_module')) {
    ...
    {
        return in_array(strtolower($module), config('niceshop.modules.core.config.core_modules'));
    }
}
...
if (!function_exists('niceshop_version')) {
    /**
     * Get the niceshop version.
     *
     * @return string
     */
    function niceshop_version()
    {
        return NiceShop::VERSION;
    }
}
...
if (!function_exists('currency_symbol')) {
    ...
    {
        if ($currencyCode == 'IRR') return locale() == 'fa' ? "تومان" : 'TOMAN';
...
if (!function_exists('locale_date')) {
    /**
     * Convert carbon date to locale date.
     *
     * @param string $currencyCode
     *
     * @return string
    ...
    function locale_date($date, $format)
    {
        if (locale() == 'fa') {
            switch ($format) {
                case "toFormattedDateString": {
                        return Jalalian::fromCarbon($date)->format('%B %d، %Y');
                    }
