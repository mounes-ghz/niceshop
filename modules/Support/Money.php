...
class Money implements JsonSerializable
{
    ...
    public function format($currency = null, $locale = null)
    {
        ...
        if ($currency == 'IRR') {
            if (strpos($amount, 'ریال') !== false) {
                $amount = str_replace('ریال','', $amount) . 'تومان';
            } else if (strpos($amount, 'IRR') !== false) {
                $amount = str_replace('IRR','', $amount) . ' TOMAN';
            } else {
                $amount = number_format($this->amount) . ' TOMAN';
            }
        }
        /**
