...
class SettingTabs extends Tabs
{
    ...
    private function sms()
    {
        return tap(new Tab('sms', trans('setting::settings.tabs.sms')), function (Tab $tab) {
            ...
            $tab->fields([
                ...
                'twilio_token',
                'ippanel_api_key',
                'ippanel_new_order_admin_sms_pattern',
                'ippanel_welcome_sms_pattern',
                'ippanel_new_order_sms_pattern',
                'ippanel_sms_order_statuses_pattern',
                'kavenegar_api_key',
                'kavenegar_new_order_admin_sms_pattern',
                'kavenegar_welcome_sms_pattern',
                'kavenegar_new_order_sms_pattern',
                'kavenegar_sms_order_statuses_pattern',
                'melipayamak_username',
                'melipayamak_password',
                'melipayamak_new_order_admin_sms_pattern',
                'melipayamak_welcome_sms_pattern',
                'melipayamak_new_order_sms_pattern',
                'melipayamak_sms_order_statuses_pattern',
                'sms_order_statuses'
            ]);
    ...
    private function payir()
    {
        return tap(new Tab('payir', trans('setting::settings.tabs.payir')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['payir_enabled', 'translatable.payir_gateway_label', 'translatable.payir_gateway_description', 'payir_merchantId']);

            $tab->view('setting::admin.settings.tabs.payir');
        });
    }

    private function zarinpal()
    {
        return tap(new Tab('zarinpal', trans('setting::settings.tabs.zarinpal')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['zarinpal_enabled', 'translatable.zarinpal_gateway_label', 'translatable.zarinpal_gateway_description', 'zarinpal_merchantId']);

            $tab->view('setting::admin.settings.tabs.zarinpal');
        });
    }

    private function zibal()
    {
        return tap(new Tab('zibal', trans('setting::settings.tabs.zibal')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['zibal_enabled', 'translatable.zibal_gateway_label', 'translatable.zibal_gateway_description', 'zibal_merchantId']);

            $tab->view('setting::admin.settings.tabs.zibal');
        });
    }

    private function sadad()
    {
        return tap(new Tab('sadad', trans('setting::settings.tabs.sadad')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['sadad_enabled', 'translatable.sadad_gateway_label', 'translatable.sadad_gateway_description', 'sadad_key', 'sadad_merchantId', 'sadad_terminalId']);

            $tab->view('setting::admin.settings.tabs.sadad');
        });
    }

    private function saman()
    {
        return tap(new Tab('saman', trans('setting::settings.tabs.saman')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['saman_enabled', 'translatable.saman_gateway_label', 'translatable.saman_gateway_description', 'saman_merchantId']);

            $tab->view('setting::admin.settings.tabs.saman');
        });
    }

    private function sepehr()
    {
        return tap(new Tab('sepehr', trans('setting::settings.tabs.sepehr')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['sepehr_enabled', 'translatable.sepehr_gateway_label', 'translatable.sepehr_gateway_description', 'sepehr_terminalId']);

            $tab->view('setting::admin.settings.tabs.sepehr');
        });
    }

    private function idpay()
    {
        return tap(new Tab('idpay', trans('setting::settings.tabs.idpay')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['idpay_enabled', 'translatable.idpay_gateway_label', 'translatable.idpay_gateway_description', 'idpay_merchantId']);

            $tab->view('setting::admin.settings.tabs.idpay');
        });
    }

    private function irankish()
    {
        return tap(new Tab('irankish', trans('setting::settings.tabs.irankish')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['irankish_enabled', 'translatable.irankish_gateway_label', 'translatable.irankish_gateway_description', 'irankish_terminalId', 'irankish_password', 'irankish_acceptorId', 'irankish_pubKey']);

            $tab->view('setting::admin.settings.tabs.irankish');
        });
    }

    private function payping()
    {
        return tap(new Tab('payping', trans('setting::settings.tabs.payping')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['payping_enabled', 'translatable.payping_gateway_label', 'translatable.payping_gateway_description', 'payping_merchantId']);

            $tab->view('setting::admin.settings.tabs.payping');
        });
    }

    private function behpardakht()
    {
        return tap(new Tab('behpardakht', trans('setting::settings.tabs.behpardakht')), function (Tab $tab) {
            $tab->weight(40);

            $tab->fields(['behpardakht_enabled', 'translatable.behpardakht_gateway_label', 'translatable.behpardakht_gateway_description', 'behpardakht_terminalId', 'behpardakht_username', 'behpardakht_password']);

            $tab->view('setting::admin.settings.tabs.behpardakht');
        });
    }
