...
@component('admin::components.page.index_table')
    @slot('buttons', ['create', 'upload_excel']) <!-- اضافه کردن دکمه آپلود اکسل -->
...
@push('scripts')
    <script>
        NiceShop.langs['admin::admin.table.pagination_start_to_end_of_total'] = '{{ trans('admin::admin.table.search_here') }}';
    ...
    <button type="button" class="btn btn-default" data-toggle="modal" data-target="#uploadExcelModal">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="uploadExcelModalLabel">{{ trans('product::products.upload_excel') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('admin.products.upload_excel') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <label for="excelFile">{{ trans('product::products.choose_excel_file') }}</label>
                        <input type="file" name="excel_file" id="excelFile" class="form-control" accept=".xlsx, .xls">
                    </div>
                </div>
